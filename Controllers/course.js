...
exports.getCourse =  (req, res) => {
  Course.findOne({_id: req.params.id}).select('-registrations').populate('subject', '-_id -__v -courses').populate('chapters')
      .then(subject => res.status(200).json(subject))
      .catch(error => res.status(404).json({error}));
}

exports.getAllCourse = (req, res, next) => {
  Course.find().select('-registrations').populate('subject', '-_id -__v -courses').populate('chapters', '_id title')
